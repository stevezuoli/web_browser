...
void ScreenProxy::sendCommand(ScreenCommand & command, ScreenCommand::WaitMode wait)
{
    // qDebug("sendCommand at %s", qPrintable(QTime::currentTime().toString("mm:ss.zzz")));
    command.wait_flags = wait;
    emit screenUpdate(command);
}
...
{
#ifndef BUILD_FOR_FB
    if (!isUpdateEnabled() && waveform != duokan::screen::ScreenProxy::DW)
    {
        return;
    }
