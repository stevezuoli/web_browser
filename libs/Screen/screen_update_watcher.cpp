...
#include "screen_update_watcher.h"
...
{
    useDwEnqueue(true);
    enqueue(widget, rc, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
}
...
bool ScreenUpdateWatcher::eventFilter(QObject *obj, QEvent *event)
{
    ...
    else if (event->type() == QEvent::WindowActivate)
    {
        if (obj->isWidgetType())
        {
            QWidget * wnd = static_cast<QWidget *>(obj);
            wnd->update();
            enqueue(wnd, duokan::screen::ScreenProxy::GC);
        }
    }
    return QObject::eventFilter(obj, event);
}
...
bool ScreenUpdateWatcher::enqueue(UpdateItem & item,
                                  QWidget *widget,
                                  duokan::screen::ScreenProxy::Waveform w,
...
void ScreenUpdateWatcher::enqueue(QWidget *widget,
                                  duokan::screen::ScreenProxy::Waveform w,
                                  duokan::screen::ScreenCommand::WaitMode wm)
{
    UpdateItem item;
...
{
    updateScreenInternal(false, duokan::screen::ScreenProxy::GU);
}

void ScreenUpdateWatcher::gcUpdateScreen()
{
    updateScreenInternal(false, duokan::screen::ScreenProxy::GC);
}

void ScreenUpdateWatcher::enableUpdate(bool enable)
{
    duokan::screen::instance().enableUpdate(enable);
}

void ScreenUpdateWatcher::updateScreenInternal(bool automatic,
                                               duokan::screen::ScreenProxy::Waveform waveform)
{
    duokan::screen::ScreenProxy::Waveform w = duokan::screen::ScreenProxy::DW;
    duokan::screen::ScreenCommand::WaitMode wait = duokan::screen::ScreenCommand::WAIT_NONE;
    QRect rc;
    ...
    if (!rc.isEmpty())
    {
        duokan::screen::instance().updateWidgetRegion(0, rc, w, false, wait);
    }
}
