...
std::string ReadStringFromFile(const char* file, bool log)
{
    std::string result;
    if (NULL == file)
    {
	...
	if (log)
	{
		qDebug("Read string from file begin: %s", file);
	}
	...
	if (log)
	{
		qDebug( "Read string from file end: %s", file);
	}
...
void DeviceInfo::GetDeviceInfoByString(string strInfo,string& outstrKey,string& outstrValue)
{
    ...
    qDebug("DeviceInfo::GetDeviceInfoByString strInfo=%s, outstrKey=%s, outstrValue=%s", strInfo.c_str(), outstrKey.c_str(), outstrValue.c_str());
...
{	
    ...
    qDebug("GetDeviceType(%s)" , subsn.c_str() );
 ...
 */
 string DeviceInfo::GetKindleVersion()
 {
	...
	string prettyversion = ReadStringFromFile(PRETTY_VERSION_FILE, false);
