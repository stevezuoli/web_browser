...
DuokanDialog::DuokanDialog(QWidget *parent, bool show_shadow)
...
DuokanDialog::~DuokanDialog(void)
{
}

void DuokanDialog::createDefaultLayout()
{
...
void DuokanDialog::updateTitle(const QString &message)
{
    title_text_label_.setText(message);
}

void DuokanDialog::updateTitleIcon(const QPixmap& pixmap)
{
    title_icon_label_.setPixmap(pixmap);
}

void DuokanDialog::showCloseButton(bool show)
{
    close_button_.setVisible(show);
}

QRect DuokanDialog::outbounding(QWidget *parent)
{
    QRect rc(rect());
    if (show_shadows_)
    {
...
void DuokanDialog::done(int r)
{
    duokan::screen::instance().ensureUpdateFinished();
    return QDialog::done(r);
}

void DuokanDialog::onClosePressed()
{
    QPixmap close_pressed_pixmap(":/images/close_pressed.png");
    close_button_.setIcon(QIcon(close_pressed_pixmap));
    duokan::screen::instance().updateWidget(&close_button_, duokan::screen::ScreenProxy::GU);
}

void DuokanDialog::onCloseClicked()
{
    shadows_.show(false);
    done(QDialog::Rejected);
}

void DuokanDialog::closeEvent(QCloseEvent * event)
{
    QDialog::closeEvent(event);
}


void DuokanDialog::moveEvent(QMoveEvent *e)
{
    if (show_shadows_)
    {
...
void DuokanDialog::resizeEvent(QResizeEvent *e)
{
    /*
    QPainterPath p;
...
void DuokanDialog::hideEvent(QHideEvent * event)
{
    QDialog::hideEvent(event);
    shadows_.show(false);
}

void DuokanDialog::keyPressEvent(QKeyEvent * event)
{
    event->accept();
}

void DuokanDialog::keyReleaseEvent(QKeyEvent * ke)
{
    ke->accept();
...
int DuokanDialog::spacing()
{
    return SPACING;
}

int DuokanDialog::defaultItemHeight()
{
    return 36;
}
