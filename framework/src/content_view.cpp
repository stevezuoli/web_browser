...
ContentView::ContentView(QWidget *parent)
        ...
        , pen_width_(3)
        , bk_color_(Qt::white)
        , waveform_(duokan::screen::ScreenProxy::DW)
{
...
void ContentView::mouseMoveEvent(QMouseEvent * e)
{
    if (data())
    {
        ...
        {
            int direction = PageTurningConfig::direction(s_mouse, e->globalPos());
            if (direction != 0)
            {
                setPressed(false);
            }
...
void ContentView::setFocusWaveform(duokan::screen::ScreenProxy::Waveform w)
{
    waveform_ = w;
}

duokan::screen::ScreenProxy::Waveform ContentView::focusWaveform()
{
    return waveform_;
}
...
void ContentView::focusOutEvent(QFocusEvent * e)
{
    ...
    emit focusChanged(this);
    //duokan::screen::watcher().enqueue(this, focusWaveform(), duokan::screen::ScreenCommand::WAIT_NONE);
}
...
void ContentView::drawTitle(QPainter &painter, QRect rect, int flags)
{
    if (data() && data()->contains(TAG_TITLE))
    {
        ...
        if (size <= 0)
        {
            size = framework::defaultFontPointSize();
        }
...
void LineEditView::createLayout()
{
    ...
    connect(&inner_edit_, SIGNAL(setCheckByMouse(DuokanLineEdit *)),
...
void LineEditView::focusInEvent(QFocusEvent * event)
{
    if (forward_focus_)
    {
        ...
        duokan::screen::watcher().enqueue(&inner_edit_, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
    }
}
...
void LineEditView::keyPressEvent(QKeyEvent * src)
{
    if (Qt::Key_Return == src->key() || Qt::Key_Enter == src->key())
    {
        ...
        src->accept();
        update();
        duokan::screen::watcher().enqueue(this,
                duokan::screen::ScreenProxy::DW,
                duokan::screen::ScreenCommand::WAIT_NONE);
    }
    else
    {
...
{
    int size = framework::defaultFontPointSize();
