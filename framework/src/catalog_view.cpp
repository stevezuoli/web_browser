...
CatalogView::CatalogView(ViewFactory * factory, QWidget *parent)
        : QWidget(parent)
        , layout_(this)
        , factory_(factory)
        , left_margin_(0)
...
void CatalogView::arrangeAll(bool force)
{
    ...
    if (isVisible())
    {
        ...
        duokan::screen::watcher().enqueue(parentWidget(), duokan::screen::ScreenProxy::GC);
    }
}
...
void CatalogView::paintEvent ( QPaintEvent * event )
{
    ...
    if (!title_.isEmpty())
    {
        ...
        font.setPointSize(framework::defaultFontPointSize() - 4);
...
{
    duokan::screen::watcher().enqueue(0, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
}
...
void CatalogView::onItemKeyRelease(ContentView *item, QKeyEvent *key)
{
    ...
    if (p)
    {
//        duokan::screen::watcher().enqueue(item, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
//        duokan::screen::watcher().enqueue(p, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
        duokan::screen::watcher().enqueue(0, duokan::screen::ScreenProxy::DW, duokan::screen::ScreenCommand::WAIT_NONE);
    }
}
...
{
    int direction = PageTurningConfig::direction(last, current);
                                           ...
                                           const QPoint &offset)
{
    ...
    {
        int d = 0;
        QPoint target_center = framework::globalCenter(target);
        QPoint item_center = framework::globalCenter(item);
        target_center += offset;
        d = framework::distance(target_center, item_center);
