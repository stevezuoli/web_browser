...
bool QTransformedScreen::connect(const QString &displaySpec)
{
    ...
    setScreen(s);
    qDebug("Screen ID:%s, QScreen:%x", dspec.toLatin1().constData(), s);
...
{
    qDebug("blit90----------rect(%d, %d, %d, %d) topLeft(%d, %d)",
...
{
    qDebug("QTransformedScreen::blit");
    ...
    qDebug("Blit Bound:(%d, %d, %d, %d)", bound.left(), bound.top(), bound.width(), bound.height());
    BlitFunc func = 0;
#ifdef QT_QWS_DEPTH_GENERIC
